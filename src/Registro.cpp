...
int calcularTamanio(const Alumno& a) {
    return sizeof(a.no_cuenta)       // 10
         + sizeof(a.telefono)        // 12
         + sizeof(a.edad)            //  4
         + sizeof(a.fecha)           //  8
         + sizeof(uint16_t)          //  2 (longitud del nombre)
         + a.nombre.size();          //  variable
}

EntradaIndice escribirRegistro(std::fstream& dat, const Alumno& a) {
    // Ir al final y capturar el offset
    ...
    // Escribir nombre variable (longitud + texto)
...
Alumno leerRegistro(std::fstream& dat, const EntradaIndice& entrada) {
    ...
    // Leer nombre variable (primero la longitud, luego el texto)
